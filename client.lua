...
local pop = nil
local waiting = false
RegisterNetEvent('lds_ui:drawtextuiwithinput')
AddEventHandler('lds_ui:drawtextuiwithinput', function(table)
...
RegisterNetEvent('lds_ui:showui')
AddEventHandler('lds_ui:showui', function(table)
    if not open then
        local coord = GetEntityCoords(PlayerPedId())
        Wait(1000)
        open = false
...
RegisterNetEvent('lds_ui:closeui')
AddEventHandler('lds_ui:closeui', function(force)
